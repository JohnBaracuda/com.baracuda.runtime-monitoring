...
namespace Baracuda.Monitoring.Core.Systems
{
    internal class MonitoringTicker : IMonitoringTicker
    {
        ...
        private void MonitoringEventsOnProfilingCompleted(IReadOnlyList<IMonitorUnit> staticUnits, IReadOnlyList<IMonitorUnit> instanceUnits)
        {
            ...
            sceneHook.gameObject.hideFlags = MonitoringSystems.MonitoringSettings.ShowRuntimeMonitoringObject
                ? HideFlags.None
                : HideFlags.HideInHierarchy;

            sceneHook.LateUpdateEvent += Tick;

            MonitoringSystems.MonitoringUI.VisibleStateChanged += visible =>
            {
                tickEnabled = visible;
                if (!visible)
                {
